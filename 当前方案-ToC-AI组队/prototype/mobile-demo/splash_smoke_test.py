...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        assert page.get_by_text("人与人先相遇，", exact=True).is_visible()
        assert page.get_by_text("人与 Agent 再共创。", exact=True).is_visible()
        assert page.get_by_text("Meet as people.", exact=True).is_visible()
        assert page.get_by_text("Build with agents.", exact=True).is_visible()
        assert splash.get_by_text("共域", exact=True).is_visible()
        assert splash.get_by_text("合拍", exact=True).count() == 0
