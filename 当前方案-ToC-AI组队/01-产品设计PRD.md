...
> 本文承接 [00-市场行业调研.md](./00-市场行业调研.md)。这是一个独立 To C 手机产品；实体 NFC 卡是可选的跨生态建联入口和线下传播媒介，不是产品主体。

## 文档信息

| 字段 | 内容 |
|---|---|
| 产品阶段 | 96 小时黑客松 MVP／需求验证前 |
| 首发场景 | 黑客松现场的找人、匹配、组队和协作启动 |
| 核心用户 | 未组队参赛者、Idea 发起人、存在角色缺口的团队 |
| 产品形态 | 移动端 H5／PWA＋AI 匹配＋双向确认＋轻量团队空间＋可选 NFC／QR 卡片 |
| 核心结果 | 从“我现在可被发现”到“双方建立关系并开始一个共同任务” |
| 本期裁决 | **Build small：只验证一条授权展示—匹配—碰触—确认—组队—启动协作的完整闭环** |
...
| 0.3 | 2026-08-29 | 冻结产品品牌 RALLY｜集结；协作护照保留为身份能力，不再作为产品名称 |
...
我们拟构建面向黑客松参与者的实时组队与协作产品 **RALLY｜集结**。用户在活动前或现场主动授权展示自己的组队状态、能力标签、项目证据、兴趣方向和团队需求；系统匹配互补的人、项目和团队缺口，并解释“为什么值得聊”。两个已经面对面交流的人可主动碰触各自绑定账户的 AI Passport，碰触本身作为双方物理授权，直接建立“真实相遇连接”并生成相互介绍；单方面读取被动 NFC 或二维码时，只能打开有限资料并发起待确认请求。

产品价值不止是交换联系方式。双方建立关系后，可以邀请对方加入团队，系统根据成员能力和项目目标生成角色覆盖、风险提示与首批三个任务，让一次线下认识直接进入协作。已经组好队的用户仍可发布“急招队友”或“项目 SOS”，向现场寻求短时技术、设计、路演或资源援助；其他用户可声明“可提供支援”，一次被双方确认解决的援助将成为新的协作证据。协作过程和项目结果未来可以反哺个人的 AI 协作身份，提高下一次匹配质量。

96 小时版本只证明一件事：

> 一个未组队用户授权展示状态后，被一个有明确能力缺口的团队发现；双方通过主动卡对卡碰触直接建立相遇连接，或经被动 NFC／QR 请求确认后建联；随后完成入队，系统生成可编辑的团队角色建议和首批任务。P0 稳定后，再增加一条“发布 SOS—接受援助—标记解决”的 P1 竖切。
...
| 现有产品没有形成完整跨阶段闭环 | 市场扫描推断／中 | 是条件性产品机会，也可能是低频、低付费 dead zone |
...
5. 证明连接能够继续转化为入队，并在 `RALLY Room｜项目启动舱` 中由 Agent 提出角色／任务建议、由成员认领调整并最终确认；
...
8. **启动而不重复维护：** RALLY Room 负责首次分工、关键变更与贡献确认；日常沟通和执行进入飞书、GitHub 等已有工具，用户不得被要求在两套任务系统重复更新状态。
...
2. 用户 B 已创建一个 AI 硬件项目，当前团队缺一名硬件开发者；
...
6. B 邀请 A 加入项目，A 确认；
7. AI 生成团队角色覆盖、一个风险提示和三个启动任务；
8. 团队接受其中一个任务，系统记录一条“有效协作连接”。
...
- **后端：** Supabase／Firebase 或等价托管服务，优先使用现成 Auth、数据库和 Realtime；
- **数据：** PostgreSQL 或等价结构化存储，关键状态必须服务端校验；
- **AI：** 一个结构化输出接口，完成匹配解释、SOS 整理和团队启动包；
...
- [ ] 用户可通过邀请码或预置活动进入；
- [ ] 最小资料包含昵称、当前状态、3–5 个能力、至少一个兴趣方向、投入时间；
...
- [ ] 硬筛选排除不可见、过期、已拉黑和缺口已满对象；
- [ ] 每个候选显示两个基于输入字段的理由和一个待确认点；
- [ ] 可从理由跳转查看对应证据；
- [ ] AI 失败时显示模板化理由，候选列表仍可用；
- [ ] 不显示伪精确成功概率。
...
- [ ] 发布者可选择 `无偿互助／实物感谢／平台积分／有偿悬赏意向`；
- [ ] 有偿悬赏意向必须填写金额、币种、交付标准和付款说明，并明确展示“平台当前不托管、不代收、不担保”；
- [ ] 96 小时版本不接支付、不冻结资金、不自动放款，也不处理发票、退款和争议；
...
|---|---:|---:|
| 主演示连续成功 | 0 | 10／10 次无阻断 |
| 重复 Connection | 未实现 | 0 |
| 未满足来源同意规则即建联／未确认即入队 | 未实现 | 0 |
| 主动双卡握手 | 未实现 | 连续 10 次只创建唯一 Connection，双端结果一致 |
| NFC／QR 到落地页 | 未实现 | 两种入口均可用，至少两类手机实测 |
| AI 失败后的主链完成率 | 未实现 | 100% 可用规则／模板降级 |
| 端到端主演示时长 | 未实现 | 3 分钟内完成 |
...
| 蓝牙附近发现／实时定位 | 权限、隐私和兼容成本高；活动邀请码足以限定范围 |
...
|---|---|---:|---|
| 一套可部署的 H5／PWA 工程 | 前端负责人 | H8 | 静态页面＋最简 API |
| 托管数据库／Auth／Realtime | 后端负责人 | H12 | 预置演示账号＋短轮询 |
| LLM 结构化输出 API | AI／后端负责人 | H36 | 规则理由＋模板启动包＋SOS 结构化表单 |
