...
APK 默认进入 Live 模式，以 `https://49.233.197.225` 作为受信任 API 地址。用户用 RALLY 演示账号和现场访问码登录后，资料、附近定位、建联、项目邀请、入队、Room 任务和刷新恢复都读写真实后端。界面资源仍内置在 APK 中，但业务操作需要可访问的 HTTPS API。
