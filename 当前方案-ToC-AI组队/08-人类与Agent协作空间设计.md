...
> **当前实现状态（2026-08-29）：** 本节是已确认的产品模型，但尚未进入运行代码。手机 Demo 只展示静态任务建议，后端尚无 Project、Membership、Leadership Term 或 Collaboration Space 表和接口。
...
空间创建者在后台保留 `owner` 权限，用于演示重置和成员访问，不需要在手机端暴露复杂管理页。后续协作空间原型只需额外证明两条规则：Agent 的重新分工只能生成待确认变更；“删除历史存档”按钮只展示多人确认流程，不执行真实硬删除。
