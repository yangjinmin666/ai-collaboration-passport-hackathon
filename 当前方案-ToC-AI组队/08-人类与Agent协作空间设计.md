...
这里的关键不是让多个模型“互相聊天”，也不是再做一套飞书、Notion 或 Linear，而是建立一个轻量的**协作控制面**：知道目标是什么、谁能做什么、谁想尝试什么、任务依赖什么、当前卡在哪里、产出由谁验收，以及结果如何回到个人可携带的协作护照。
...
`Collaboration Space` 是围绕一个明确项目目标创建的临时任务空间。它包含：
...
1. 刚组好的队伍不知道谁负责什么；
2. 任务拆出来了，但依赖、承诺和完成标准不清楚；
3. 人和 AI 工具各自在工作，却没有统一的任务状态和交付物入口。
...
96 小时版本只实现“项目发起人在单人阶段担任 Bootstrap Coordinator；首批成员入队后确认一名 Coordinator；界面始终独立展示发起归属”，不做复杂任期选举和方向 Fork 交互。
...
空间创建者在后台保留 `owner` 权限，用于演示重置和成员访问，不需要在手机端暴露复杂管理页。Demo 中只需额外证明两条规则：Agent 的重新分工只能生成待确认变更；“删除历史存档”按钮只展示多人确认流程，不执行真实硬删除。
...
协作空间属于底部导航的“协作”模块，不新增第四个一级 Tab。
...
13. 提交一个演示交付物并由成员确认完成；
14. 将结果写为一条私有协作记录；
15. 演示历史存档删除需要多名成员确认，但不执行真实硬删除。

在现有 Demo 中，`AI 团队启动包` 就是协作空间的最小版本，不需要另做一套完整项目管理页面。
...
| V1：任务控制面 | 方向来源、依赖、阻塞、交付物、审核、重新路由 | 团队是否愿意持续在此维护协作状态与归属记录 |
...
| V3：跨工具协作 | GitHub、Figma、飞书等结果引用与事件同步 | 产品能否成为已有工具之上的协调层 |
...
> 飞书、Notion、GitHub 和 Figma 负责承载具体沟通与产出；我们的产品负责把“刚认识的一群人”变成有目标、有承诺、有任务路由和可验证结果的协作团队。
