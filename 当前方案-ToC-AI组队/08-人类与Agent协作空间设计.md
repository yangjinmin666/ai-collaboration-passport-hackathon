...
> 本文承接 [06-产品定位与差异化.md](./06-产品定位与差异化.md) 与 [07-手机端功能结构与信息架构.md](./07-手机端功能结构与信息架构.md)。它回答：两个人通过发现或碰卡建立关系以后，产品怎样把“认识”继续推进为“人和 Agent 一起完成项目”。

> **范围声明：** 本文是碰卡建联之后的赛后 V1 产品蓝图。96 小时主线仍以 PRD 中的“授权展示—发现—碰卡／确认—入队—三任务启动包”为冻结范围；本文的 Coordinator、改派审批和历史删除多人确认均不是本期必交付项。

> **2026-08-29 边界更新：** “协作空间”正式收窄并命名为 `RALLY Room｜项目启动舱`。它不是长期执行平台，只负责团队形成、首次分工、关键变更与贡献确认；日常沟通、文档和代码执行继续进入飞书、GitHub 等已有工具。本文后续治理规则应理解为启动舱的治理内核与长期数据模型，而不是要求在 RALLY 内重做完整协作软件。
...
> **AI 帮一群刚刚建立关系的人看清任务、选择分工，并协调人和 Agent 一起把事情做完。**

碰卡建立的是一条真实关系，协作空间承接的是这条关系接下来要共同完成的目标。二者不是两个孤立功能：
...
→ 人确认承诺
→ 将已确认方案同步到既有执行工具
→ 关键变更、项目结果与贡献证据回写 RALLY
```

这里的关键不是让多个模型“互相聊天”，也不是再做一套通用协作软件，而是建立一个轻量的**启动与治理层**：知道目标是什么、谁愿意负责什么、Agent 为什么这样建议、团队确认了哪个版本、何时发生重大变化，以及结果如何回到个人可携带的协作护照。

### 1.1 原有治理设计如何保留

| 处理方式 | 设计内容 | 在新边界中的位置 |
|---|---|---|
| V0 直接保留 | Agent 只有建议权、成员自主认领、人类最终确认、重大变更多人确认、历史可追溯 | RALLY Room 的核心交互 |
| 保留数据模型 | 项目／方向发起归属、方向版本、Leader 任期、贡献证据 | 关键记录与赛后协作护照 |
| 推迟产品化 | 复杂权限矩阵、完整任务生命周期、交付物审核合并、长期 Agent 调度 | 等真实团队证明需要后再做 |
...
`RALLY Room` 是围绕一个明确项目目标创建的临时启动与治理空间。它包含：

- 已确认加入的成员与 Agent；
- 项目目标、时间窗口和验收标准；
- 成员授权使用的能力信息、兴趣方向、成长目标与可投入时间；
- 首次分工所需的任务、候选负责人、风险和完成标准；
- Agent 提出的路由建议及其理由；
...
> **当前实现状态（2026-08-29）：** 薄闭环基线已经进入运行代码：后端具有 `Project`、角色缺口、`ProjectMembership`（`ORIGINATOR／LEADER／MEMBER`）、`StarterPack`、三项启动任务、成员本人认领、全员方案确认、RALLY Room 聚合视图和项目 Activity。尚未实现 `LeadershipTerm` 选举／交接、方向谱系、重大变更多人确认、历史归属更正和外部工具双向同步；因此当前 `ORIGINATOR` 是启动阶段的治理占位，不应被解释为永久 Leader。
...
flowchart TD
    ...
    E --> F["提出候选人、理由与风险"]
    F --> G{"Leader 选择且本人接受？"}
    G -->|"自主调整／认领"| H["修改分工或协作方式"]
    H --> F
    G -->|"接受"| I["人和 Agent 并行执行"]
    I --> J["提交交付物"]
    J --> K{"验收通过？"}
    K -->|"返工"| I
    K -->|"通过"| L["合并成果并回写贡献证据"]
...
这条流程中有三个不同层次的同意，不能合并为一次授权：

1. **碰卡同意建联：** 允许建立真实相遇连接；
2. **入队同意：** 允许加入一个具体项目空间；
3. **任务同意：** 团队可以选择分工，但被指派者仍需确认是否承担；对于 Agent，则由人确认其可使用的资料、工具和执行范围。
...
`RALLY Room` 属于手机底部导航的“协作”模块；“连接”仍作为独立一级入口，保存跨项目、跨活动的人际关系。启动舱采用同一数据、两种终端形态：
...
确认完成后，团队进入已有工具执行。RALLY 不要求成员在两个平台重复维护日常状态；只有关键变化和项目结果需要回写。

界面应延续手机端的黑白灰＋少量蓝色：蓝色只用于当前主行动、待确认和选中状态；不要用大面积颜色表达“AI 正在工作”，也不要依赖颜色区分任务状态。

## 10. RALLY Room 分阶段演示范围
...
2. 一方发出项目邀请，另一方确认加入；
3. 系统创建轻量 `RALLY Room`；
4. 界面分别显示“项目发起人”和“当前 Coordinator”；发起人在单人阶段负责启动，成员入队后共同确认维护人；
5. Agent 读取项目目标和两人的已授权能力；
6. 生成角色覆盖、一个风险和三个任务；
7. 每个任务显示执行类型、候选负责人、推荐理由、风险和完成标准；
8. 只有协调人可以确认正式候选负责人和调用共享 Agent；
9. 其中一名成员对一个“非最优推荐”的任务表达“我想做”；
10. 协调人批准改派，并选择独立、结对或 Agent 辅助方式；
11. Agent 再次建议改派时，只产生一张带影响范围的待确认变更卡，不直接改变现有分工；
12. 被指派者明确接受后，任务才进入“进行中”；
13. 生成飞书／GitHub 同步包，说明执行离开启动舱；
14. 将一个项目结果写为私有协作记录；
15. 演示历史存档删除需要多名成员确认，但不执行真实硬删除。
...
| V1：关键治理层 | 方向来源、关键阻塞、重大变更、结果回写 | 团队是否愿意在启动和关键节点回到 RALLY，而非维护第二套任务系统 |
| V1.5：方向谱系 | Direction Version、Pivot／Fork、方向发起归属和 Leadership Term | 方向转型时能否同时保留旧发起归属与新方向贡献 |
| V2：Agent 执行接入 | 接入一个 Coding Agent 或文档 Agent，权限可控、结果可审查 | Agent 是否降低实际执行时间，而非只增加演示感 |
...
> 飞书、GitHub 等已有工具负责承载具体沟通与产出；RALLY 负责把“刚认识的一群人”变成完成首次分工、能够立刻开工的团队，并沉淀关键变化与真实贡献。
